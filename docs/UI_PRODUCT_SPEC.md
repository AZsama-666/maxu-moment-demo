...
| 步 | 路由 | 标题 | 要点 |
|----|------|------|------|
| 1 | `/launch/type` | 选择 SKU | 1V1 语音/视频/**组局·狼人杀**/陪玩 · 平台履约/双方确认 · 组局仅 1 listing |
| 2 | `/launch/product` | 商品与定价 | 标题·介绍·价格·时长(1V1)或席位(组局/陪玩) · **组局 Step2 预填狼人杀** |
| 3 | `/launch/fulfillment` | 履约设置 | 1V1：开放可约·营业时间；**组局：预填时间/地点/简介/微信/退改**；陪玩：服务时间·地点 |
| 4 | `/launch/preview` | 确认发布 | 1V1/陪玩 Moment 卡；**组局 GroupCard** · **确认发布** |

**发布成功** `/launch/success/:id`（Tab **显示**）：组局→**管理本活动**+查看买家详情；1V1→待处理任务/等待室

**组局 Tab 曝光**：`listBrowseGroupListings` 仅 dynamic（隐藏静态 g-boardgame/g-coffee），发布后才在 Tab 出现
